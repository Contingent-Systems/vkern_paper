...
pte_t *pte_get_next_table(pte_t *entry) {
  ...
  if (!entry->present(*\label{line:read_entry_contentsC}*) (*\label{line:conditional_childrenC}*)) {
    ...
    entry->pfn = nextpaddr; // Point to next table
...
\end{minipage}
%\end{framed}
\todo[inline,color=cyan]{delete line 6 from this figure and later get next table code}
    \vspace{-2.5em}
    \caption{A kernel implementation of x86-64 page table lookups.}
