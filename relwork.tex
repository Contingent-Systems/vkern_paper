...
with an idiosyncratic format. \replace{It turns out that s}{S}ubsequent OS verification work also treats page tables this way, but
unsoundly given that \replace{other projects}{their} target hardware \del{that} \emph{does} run the kernel with address translation: they \emph{trust}
...
of the L1 entry (which they assumed existed), thereby skipping the physical-to-virtual conversion issues we treat in Section ??.
 ...
 We have verified significantly more VMM code than Kolanski and Klein, which pushed us towards the modal approach.
 They verify~\cite{kolanski08vstte} roughly the (ARM) assembly corresponding to Lines \ref{line:l1entry_storeC} and \ref{line:l1entry_setpresent} of Figure
 \ref{fig:mapping_codeC},
 plus pulling the zeroed page to install in the L1 entry from a simple linked list.
 They never addressed code equivalent to our \lstinline|walkpgdir| (Figure \ref{walkpgdirC}), or \lstinline|pte_get_next_table| (Figure \ref{fig:calltopteinitializeC}),
 which are the most challenging parts of verifying a full page table walk, encompasing subtle sharing and ownership
 specifications and delicate kernel invariants involved in converting physical to virtual addresses. Ours is the only work to date
 to tackle verification of the \emph{entirety} of a software page table walk and mapping a new page.
}
