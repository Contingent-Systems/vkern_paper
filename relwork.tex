...
% \footnote{\add{A ``fun'' bug one author encountered in
...
Other work on OS verification either never progressed \del{far enough} to \del{address} VMM verification
...
\replace{With our general approach to virtual points-to assertions being inspired by Kolanski and Klein, \emph{both}
 ...
 } be extended to account for pageable points-to assertions by adding \del{additional}
disjunctions to an extended points-to definition\add{,}\del{; embedding ``regular'' separation logic into such a variant
is} \add{which would be}
...
\del{Translation Lookaside Buffers}
