...
\end{mathpar}
\caption{Small-Step Operational Reduction Rules for Light-Weight Integration of \textsf{x}86-64 Machine Model}
...
\inferrule[StepMovRR]{
  ...
  \hs{\ising{\imov\mvrr\rgsrc\rgdst}}\store
     \iskip{\store'}
}\qquad
\inferrule[StepMovRMOff]{
  \amodeo\imreg\offs \; \readlval\rgsrc\storereg\rvsrc \\\\
 \readlval\imreg\storereg\maddr \\\\
          \readlval{\plusaddr\maddr\offs}{\storememstar\crval}\locsf \\\\
         \readlval{\plusaddr\maddr\offs}{\storememprimestar\crval}\rvsrc
}{
  \hs{\ising{\imov\mvrmo\rgsrc\amode}}\store
     \iskip{\store'}
}\qquad
\inferrule[StepMovRMBase]{
 ...
 \readlval\imreg\storereg\maddr \\\\
  ...
  \hs{\ising{\imov\mvrmb\rgsrc\amode}}\store
     \iskip{\store'}
}\\
\inferrule[StepMovMROff]{
 ...
 \readlval\imreg\storereg\maddr \\\\
  ...
  \hs{\ising{\imov\mvmro\amode\rgsrc}}\store
     \iskip{\store'}
}\qquad
...
\end{mathpar}
\caption{Small-Step Relations for Chosen Instructions in \textsf{x}86-64 Model}
