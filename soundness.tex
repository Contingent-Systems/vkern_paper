...
\end{array}
\]
\todo[inline,color=yellow]{Colin, incase needed,not proven, could you simply say that we assume these map equalities}. Since the focus of this paper is not explaining our \textsf{AMD64} model, we prefer not to get into the details due to space limits. However, Coq formalization of our operational semantics is a part of our submission artifact.
...
% Lemma wexploit_table_traversal (r:greg) maddr entry entry0 entry1 paddr cr3val  (v:word 64) σ :
 % x64h_interp  σ.(mem)  σ.(reg) -∗
  %     ((cr cr3) ↦ᵣ (num cr3val)) cr3val -∗
   %    (r ↦ᵣ (num maddr)) cr3val -∗
    %   (wtraversal_mapsto maddr entry entry0 entry1 paddr v) cr3val
