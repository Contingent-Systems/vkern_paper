...
Definition exec_step (i: instr) ($\sigma$:state) : option state :=
    exec_state (exec_instr i $\sigma\mathcal{C}$ $\sigma.\mathcal{R}$ ((memToPhysMem $\sigma.\mathcal{M}$) (exist _ ($\sigma.\mathcal{R}$ !!  cr3)))).

\end{lstlisting}
\todo[inline,color=yellow]{Colin, in case needed,not proven, could you simply say that we assume these map equalities or a paper proof etc.}. Due to the page limitation, we can only give the outline of a selected instruction (\TirNameStyle{WriteToRegFromVirtMem}) from our \textsf{AMD64} model. However, mechanized soundness proofs of other instructions can be found as a part of our submission artifact.
