...
\todo[inline]{TODO: resolve entry+KERNBASE question from chat, add information about rax holding \lstinline|entry_val & 0x1| and the conditional
outcome that that is 0 in the then branch}
...
Line \ref{line:check_entry_present_jump}'s conditional then determines in the fall-through case that the bit is not set, which resolves $\textsf{qfrac} = 1$,
and permits the new entry to be installed (in ellided code around Line \ref{line:install_new_entry})
