...
;; pte_t *next;
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{entry}\},m)  \ast \texttt{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast  \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{next} \ast \texttt{r8}  \mapsto_{\textsf{r}} \textsf{\_} \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{\_}  \ast \ghostmaptoken{\textsf{id}}{\textsf{entry}}{\textsf{v}} \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
...
$\specline{\textsf{entry\_present(l4e\_val)} \wand \forall_{i\in\textsf{0..511}} \; \ghostmaptoken{\textsf{id}}{\textsf{table\_root(pml4.pfn)+ i * 8}}{\textsf{3}} }_{\rtv} $
...
The precondition provides knowledge that the virtual base of the L4 is at the appropriate offset from the current \lstineline|cr3| value,
...
pulls that entry out of the identity mapping ($\Xi$) and fetches its 

\todo[inline]{Colin is unsure:
It looks like the call to pte-get-next-table on line 19 is done after using the idmap ($\Xi$) token for the entry's physical address.
But the verification of pte-get-next-table starts by assuming it has a vpte-pointsto \emph{and} the token!
Except to build the vpte-pointsto, we had to remove that from the invariant around $\Xi$; 2/3 of the resources membership in $\Xi$
entails were already consumed to build it. That's why we have the entry-present-implies-l3-tokens
assertion on line 14 of Figure 11.
So there's a misalignment of these specs. It seems like pte-get-next-table should not get the token for the physical address, but should instead get
the conditional assertion at line 14 directly?
}


In the very first precondition on the client side -- e.g.\textsf{pte\_walkpgdir} in Figure \ref{walkpgdir} -- 
we see that \textsf{vpte-ptsto} for the entry being accessed (\textsf{entry}) is constructed -- in 
Specification Line \ref{line:get_next_vpte_precondition} of Figure \ref{fig:calltopteinitialize} for \textsf{entry}. 
Intuitively speaking, we start traversal from a certain entry address at the current level for which we know 
($\textsf{entry} \mapsto_{\textsf{id}} \textsf{\_}$ in Line 3 in Figure \ref{walkpgdir}) that 
there is a memory mapping ($\textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry entry\_val}$ 
Line 15 in Figure \ref{walkpgdir}) that can be used to explore deeper in the page-tree.
...
random entry value, subtly, the operation also, at least, requires that the relevant table entry is readable, but the exact portion of ownership 
