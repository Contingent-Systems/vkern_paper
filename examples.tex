...
\[
\begin{array}{l}
  \mathcal{I}\textsf{ASpace}(\ptablestore,\Xi,m)\stackrel{\triangle}{=} \textsf{ASpace\_Lookup}(\ptablestore,\Xi,m) \ast \\
  ...
  \qquad\qquad\qquad \bigast{(\textsf{paddr}) \in \Xi}{\exists\; (\textsf{qfrac, val, vaddr}) \ldotp \ulcorner \textsf{vaddr} = \textsf{paddr} + \textsf{KERNELBASE} \urcorner \ast  \underbrace{\sumwalkabs\vaddr\qfrac\paddr }_\text{Ghost translation} \ast \\
\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad    \underbrace{\paddr \mapsto_{\mathsf{p}}\{\textsf{qfrac}\}\; \vale}_\text{Physical location} \ast \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner }\\
  \textsf{ where } \\
   ...
   \ownGhost\gammaPred{\authfull{\ptableabswalk\ptablestore}} \ast \\ \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad \ownGhostpv\gammaPred{\authfull{\pvmapping\Xi}}
...
Work to verify the full page table walk is ongoing, but we have verified the key step for a small x86-64 kernel, which is the physical-to-virtual
...
though the Hoare triple we verify for this add instruction is rather complex:



\begin{figure}\footnotesize
\begin{lstlisting}[mathescape]
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}(\theta,\Xi,m) \ast \ulcorner \textsf{KERNELBASE} + \fpaddr > 0 \urcorner }_{\rtv}$
$\specline{ \texttt{rip}\mapsto_{\textsf{r}} \textsf{iv} \ast \texttt{rflags}\mapsto_{\textsf{r}} \textsf{flaglist} \ast \texttt{rbx}\mapsto_{\textsf{r}} \fpaddr  }_{\rtv}$
$\specline{ \sumapaces\rtv\delta  \ast \sumwalkabs\vaddr\qfrac\fpaddr \ast \sumpv\fpaddr\qfrac\true  }_{\rtv}$
;; This offsetst physical address at KERNELBASE!
add rbx, KERNELBASE
$\specline{\exists \textsf{flaglistadd, qphysical, q0, w, flaglistadd, val}, \textsf{P} \ast \texttt{rip}\mapsto_{\textsf{r}} \textsf{iv+7} \ast \texttt{rflags}\mapsto_{\textsf{r}} \textsf{flaglistadd} \ast \texttt{rbx}\mapsto_{\textsf{r}} \fpaddr + \textsf{KERNELBASE} }_{\rtv}$
$\specline{\bigast{(\vaddr, \textsf{paddr})\in \ptablestore}{\exists\;(\textsf{l4e, l3e, l2e, l1e, } \fpaddr)\ldotp \textsf{L}_{4}\_\textsf{L}_{1}\_\textsf{PointsTo}(\vaddr\textsf{, l4e, l3e, l2e, l1e, \fpaddr})}}_{\rtv}$
$\specline{\mathcal{R} -\ast \bigast{(\fpaddr) \in \Xi}{\exists\; (\textsf{qfrac, val, vaddr}) \ldotp \ulcorner \textsf{vaddr} = \fpaddr + \textsf{KERNELBASE} \urcorner \ast \sumwalkabs\vaddr\qfrac\fpaddr  \ast \paddr \mapsto_{\mathsf{p}}\{\textsf{qfrac}\} \; \vale\ast  \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner}}_{\rtv}$
$\specline{ \ownGhost\gammaPred{\authfull{\ptableabswalk\ptablestore}} \ast \ownGhostpv\gammaPred{\authfull{\pvmapping\Xi}}}$
$\specline{ \sumapaces\rtv\delta  \ast\sumwalkabs\vaddr\qfrac\fpaddr \ast \sumpv\fpaddr\qfrac\true \ast \textsf{P}}_{\rtv}$
$\specline{\vaddr \mapsto_{\texttt{vpte}} \{\textsf{q}, \textsf{qfrac}\} \paddr \;(\texttt{val})}$
\end{lstlisting}
\caption{Identity mapping of a physical address to a virtual one.}
\label{fig:mapping_code}
\end{figure}

We can see that this specification neatly encapsulates reasoning about invariants on the page tables in terms of general modal assumptions.\todo{speculative
but ought to turn out this way}
...
\begin{figure}\footnotesize
\begin{lstlisting}[mathescape]
;; Assume the save-space is in rdi, load-space in rsi
;; First, save yielding context
...
mov rbx, 0[rsi] 
;; This switches to a new stack, which may not be mapped in the current address space!
mov rsp, 8[rsi] 
... ;; mov rbp, r12, r13, r14, to offsets 16, 24, 32, and 40 from rsi
mov r15, 48[rsi]
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}(\theta,m) \ast [\rtv'+56](\mathcal{I}\texttt{ASpace}(\theta,m) \ast \texttt{Pother})}_{\rtv}$
