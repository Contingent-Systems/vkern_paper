...
Finally, we call \textsf{get\_next\_table} to get the virtual address to the next level page-table -- at this point the address of L3 table.
...
\begin{figure}\footnotesize
  \begin{lstlisting}[mathescape]
;;if(entry->present)
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{\_} \ast rdi  \mapsto_{\textsf{r}} \textsf{\_} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}$
mov    -0x8[rbp],rdi
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{\_} \ast rdi  \mapsto_{\textsf{r}} \textsf{entry} \ast \textsf{entry} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}$ 
mov     rdi, r8
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{entry} \textsf{\_} \ast rdi  \mapsto_{\textsf{r}} \textsf{entry} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}$ 
mov    [r8],rdi
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{entry} \textsf{\_} \ast rdi  \mapsto_{\textsf{r}} \textsf{entry} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}$ 
  \end{lstlisting}
\vspace{-1em}
...
\label{fig:loadentry}
\end{figure}
As we see in Figure \ref{fig:loadentry}, we have the physical address (\textsf{entry}) of the page table entry kept in the register $r8$, but not the memory mapping to load the entry value referenced by the physical address \textsf{entry}. 
...
\caption{Global Address-Space Invariant in Figure \ref{fig:peraspaceinvariant} extended with a ghost map bookkeeping identity mappings }
  ...
  \end{figure*}
Now, to read the value referenced by physical address \textsf{entry} in Figure \ref{fig:loadentry}, first, we open our the addresss-space invariant ($\mathcal{I}\textsf{ASpace}_{\textsf{id}}(\ptablestore,\Xi,m)$), and obtain the virtual mapping out of the identity mapping $\Xi$ with the side condition
\[
 \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner
 \]
 assuring that looking up identity mappings are safe, because the full ownership asserts the non-presence of the entry which requires allocation a new physical page and setting the presence bit. By obtaining the identity mapping for \textsf{entry} in Figure \ref{fig:loadentry} -- ($\textsf{entry-KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry\_val}$ with page addr \textsf{entry}),
 \[
