...
\todo[inline]{TODO: resolve entry+KERNBASE question from chat, update all rdi points-tos after line \ref{line:pass_addrof_next}}
...
$\specline{ \forall_{i\in \textsf{0 ... 511} } \ldotp  \ghostmaptoken{\textsf{id}}{((\mathsf{entry\_page}\;\textsf{entry}) + \textsf{i * 8})}{\textsf{\_}} \ast  \textsf{qfrac} = 1 \land \lnot(\textsf{entry\_present entry\_val }_{\textsf{pfn}} + \textsf{i * 8})  }$
... ;;entry value updates: entry->pfn = next; entry->present = 1; (*\label{line:install_new_entry}*)
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus \{ \mathsf{entry} \},m)  \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast \texttt{r8}  \mapsto_{\textsf{r}} \textsf{entry}  \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry\_val}  }_{\rtv}$
...
$\specline{ \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{entry}_{\textsf{pfn}}  \ast \texttt{rax} \mapsto_{\textsf{r}} \textsf{nextpaddr}  }_{\rtv}$
$\specline{ \forall_{i\in \textsf{0 ... 511} } \ldotp \ghostmaptoken{\textsf{id}}{((\mathsf{entry\_page}\;\textsf{entry}) + \textsf{i * 8})}{\textsf{\_}} \ast  \textsf{qfrac} = 1 \leftrightarrow \lnot(\textsf{entry\_present entry\_val }_{\textsf{pfn}} + \textsf{i * 8})  }$
$\sqsubseteq$
...
$\specline{ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \mathsf{entry entry\_val} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \lnot(\textsf{entry\_present entry\_val})  \urcorner }_{\rtv}$
$\specline{ \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{entry}_{\textsf{pfn}}  \ast \texttt{rax} \mapsto_{\textsf{r}} \textsf{entry}_{\textsf{pfn}}  }_{\rtv}$ (*\label{line:alloc_path_end}*)
;;}
...
However, implementing this virtual-to-physical (V2P) translation ironically requires physical-to-virtual (P2V) translation,
...
$\Xi$ is another ghost map, from physical addresses to the unit type (so, effectively a ghost set). 
...
\begin{array}{l}
  ...
  \bigast{(\paddr,\mathsf{level}) \in \Xi}{\exists\; (\textsf{qfrac, q, val,}\vaddr) \ldotp \ulcorner \vaddr = \paddr + \textsf{KERNBASE} \; \textsf{level} > 1\urcorner \ast  \underbrace{\fracghostmaptoken{\delta}{\vaddr}{\paddr}{\qfrac} }_\text{Ghost translation} \ast \underbrace{\paddr \mapsto_{\mathsf{p}}\{\textsf{qfrac}\}\; \vale}_\text{Physical location}} \ast\\
   \qquad\underbrace{ \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner}_\text{Entry validity}\ast 
    \underbrace{\left(\ulcorner\textsf{entry\_present}(\vale)\urcorner \wand \forall_{\textsf{i}\in\textsf{0..511}} \ldotp \ghostmaptoken{\textsf{id}}{((\mathsf{entry\_page}\;\vale) + \textsf{i * 8})}{\textsf{level-1}}\right)}_{\text{Indexing into next level of tables}} \\
  \textsf{ where } \\
...
and for valid entries (with the present bit set), ghost map tokens for every entry in the table pointed to by the entry.

\todo[inline,color=green]{Colin says: 
The indexing into the next level of tables is a little weird at the L1 level. The page pointed to by an L1 entry will be a data
page. We don't actually need to perform P2V on data pages in general. But also problematic is that this invariant implies
partial ownership of the data (by taking the $\Xi$ tokens for the data page back to this invariant). So we need a way
to make this ``bottom out'' after the L1 entries. Something like switching $\Xi$ to be a map from a physical address to the corresponding level of tables
(as a natural number; we'll always start out knowing that $\ghostmaptoken{\textsf{id}}{\textsf{cr3val}}{4}$ for a traversal),
and then switching the indexing into the next level to be
\[
\bigast{(\paddr,\mathsf{level})}{\ldots\left(\ulcorner\textsf{entry\_present}(\vale)\land \mathsf{level} > 1 \urcorner \wand \forall_{\textsf{i}\in\textsf{0..511}} \ldotp \ghostmaptoken{\textsf{id}}{((\mathsf{page\_addr}\;\vale) + \textsf{i * 8})}{(\mathsf{level}-1)}\right)}\]
I'm surprised this hasn't come up during page table setup
}
...
Thus the fractional ownership of the physical location is enough for Line \ref{line:read_entry_contents} to access the entry, though in \lstinline|get_next_table|
the caller has pulled that piece of information out of the invariant and passed it for the entry at hand, which is why this appears explicitly in assertions,
and the argument to the invariant is $\Xi\setminus\{\mathsf{entry}\}$\todo{update pending entry+KB} (indexing by the set $\Xi$ allows us to borrow the physical resources
...
\todo[inline,color=blue]{Colin revision frontier: need to resolve entry+KERNBASE issue to finish pulling this into prose above}
...
;;pte_t *local = kalloc();
callq  81 <pte_initialize+0x11>	7d: R_X86_64_PLT32	kalloc-0x4 (*\label{line:call_to_kalloc}*)
mov    rax,-0x10[rbp] 
;;entry->pfn = PTE_ADDR_TO_PFN((uintptr_t) local);
mov    -0x10[rbp],rax
mov    -0x8[rbp],rdi
mov    rax,[rdi]
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m)   \ast \texttt{rbp-16} \mapsto_{\textsf{r}} \textsf{\_} \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry+KERNBASE} \ast \texttt{rax} \mapsto_{\textsf{v}} \textsf{nextpaddr}  }_{\rtv}$
$\specline{\textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,}} \textsf{entry entryv} \ast \ghostmaptoken{\textsf{id}}{(\mathsf{entry})}{\textsf{level}} }$
$\specline{\texttt{rax} \mapsto_{\textsf{v}} \textsf{nextpaddr} \ast \lnot(\textsf{entry\_present entry\_val}) }$
$\specline{ \textsf{entry+KERNBASE} \mapsto_{\mathsf{pte,1}} \mathsf{entry} \; \textsf{pfn\_set(entryv nextpaddr)}}$
$\specline{\ghostmaptoken{\delta{}s}{\rtv}{\delta} \ast \begin{array}{l} \textsf{entry\_present(pte\_initialize(pfn\_set(entryv nextpaddr)))} -* \\  \;\;\;\;\;\;\; \forall_{i\in\textsf{0..511}} \; \textsf{table\_root(pte\_initialized (pfn\_set (entryv nextpaddr))) + i * 8} \mapsto_{\textsf{id}} \textsf{level-1}    \end{array}  }_{\rtv}$
;;clean up the stack
...
\end{lstlisting}
\vspace{-1em}
...
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus \{ \mathsf{entry} \},m)  \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast \texttt{r8}  \mapsto_{\textsf{r}} \textsf{entry}  \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry\_val}  }_{\rtv}$
$\specline{ \textsf{entry} \mapsto_{\textsf{id}} \textsf{\_} \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
$\specline{ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry entry\_val}  }_{\rtv}$
$\specline{  \textsf{entry}_{\textsf{pfn}} \textsf{+KERNBASE}  \mapsto_{\textsf{vpte,qfrac}}   \textsf{entry}_{\textsf{pfn}} \; \textsf{entry\_pfn\_val} \ast R_{\textsf{children}}}_{\rtv}$
$\specline{ \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{entry}_{\textsf{pfn}}  \ast \texttt{rax} \mapsto_{\textsf{r}} \textsf{entry}_{\textsf{pfn}} \ast \textsf{entry}_{\textsf{pfn}} \mapsto_{\textsf{id}} \textsf{\_}}_{\rtv}$
...
;;next = (pte_t *) next_virt_addr;
;;clean up the stack and return next
...
$\specline{\begin{array}{l} \textsf{entry\_present(l4e\_val)} -* \\  \;\;\;\;\;\;\; \forall_{i\in\textsf{0..511}} \; \textsf{table\_root(plm4)}_{pfn} \textsf{ + i * 8} \mapsto_{\textsf{id}} \textsf{3}    \end{array}} $
...
$\specline{\texttt{rax}  \mapsto_{\textsf{r}} \textsf{table\_root(pml4\_entry}_{\textsf{pfn}})}$
;;pte_t *pdp_entry = &pdp[PDPEX(va)];
$\specline{\textsf{pdp}+\textsf{PDPEX(va)} = \textsf{pdp\_entry} \land \textsf{table\_root(pml4\_entry}_{\textsf{pfn}})  = \textsf{pdp}}$
...
$\specline{\begin{array}{l} \textsf{entry\_present(l3e\_val)} -* \\  \;\;\;\;\;\;\; \forall_{i\in\textsf{0..511}} \; \textsf{table\_root(pdp\_entry}_{\textsf{pfn}}) \textsf{ + i * 8} \mapsto_{\textsf{id}} \textsf{2}    \end{array}} $
$\specline{\ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l3e\_val})\urcorner}$
