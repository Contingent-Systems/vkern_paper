...
the first such verification handling both old and new processes' assertions at the time of the switch.
...
Note that the specification does \emph{not} assume a specific page table --- logical parameter \textsf{v} represents the level
...
% }
(a virtual points-to exposing the underlying physical address; see Section \ref{sec:mapnew}) granting access to the specified entry,
...
The assertion on Line \ref{line:conditional_children} comes from the invariant one level up; 
...
allocates a physical page (internally utilizing the only axiomatized call in our case studies, the page-allocator's \textsf{kalloc}, 
...
\begin{figure}\footnotesize
  \vspace{-1em}
$\begin{array}{l}\left( \begin{array}{l} \ulcorner \textsf{pdp}+\textsf{PDPEX(va)} = \textsf{pdp\_entry} \land \textsf{table\_root(pml4\_entry}_{\textsf{pfn}})  = \textsf{pdp} \\ \land \textsf{pd}+\textsf{PDEX(va)} = \textsf{pd\_entry} \land \textsf{table\_root(pml3\_entry}_{\textsf{pfn}})  = \textsf{pd} \\ \land \textsf{pt}+\textsf{PDPEX(va)} = \textsf{pt\_entry} \land \textsf{table\_root(pml2\_entry}_{\textsf{pfn}})  = \textsf{pt} \urcorner  \ast \\  \textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{pml4\_entry},\textsf{pdp\_entry},\textsf{pd\_entry},\textsf{pte}\},m)  \\ \ghostmaptoken{\delta{}s}{\rtv}{\delta} \ast  \\
      \ghostmaptoken{\textsf{id}}{(\mathsf{pml4\_entry})}{\textsf{4}}\ast \ghostmaptoken{\textsf{id}}{(\mathsf{plm3\_entry})}{\textsf{3}} \ast \ghostmaptoken{\textsf{id}}{(\mathsf{plm2\_entry})}{\textsf{2}}  \ast \\  \textsf{pml4\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l4e\_val}  \ast \\  \textsf{pdp\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l3e\_val}  \ast \\  \textsf{pd\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l2e\_val}  \end{array} \right)  =\textsf{R}_{\textsf{walk} } \\ \ast \left(\begin{array}{l} \textsf{rax} \mapsto \textsf{pte} \ast  \textsf{pt\_entry} \mapsto_{\textsf{vpte,qfrac}} \textsf{pte} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present pte\_val})\urcorner \end{array}\right) =\textsf{R}_{\textsf{l1e}}\end{array} $
\caption{\textsf{R}$_{\textsf{walk}}$: Resources obtained from invariant for walking page-table directory with \emph{non-existing entries} via calls to \textsf{pte\_get\_next\_table} in Figure \ref{fig:calltopteinitialize}}
\label{fig:rwalk}
\vspace{-1em}
\end{figure}
...
\[\textsf{vaspace\_mappage(pte\_t *pml4, void *va,uintptr\_t fpaddr)}\] in Figure \ref{fig:mapping_code}.
...
;; pte_t *pte = walkpgdir(pml4, a);
 ...
 mov  -0x16[rbp], rsi;
 mov  -0x24[rbp], rdx;
;; pte = walkpgdir_succ(pml4, va);
 mov    -0x8[rbp], %rdi;
 mov    -0x16[rbp], rsi;
 mov    0x0, rax;
 callq  291 <walkpgdir> (*\label{line:call_walkpgdir}*)
 $\specline{ \textsf{R}_{\textsf{walk}} \ast \left(\begin{array}{l}   \textsf{rax} \mapsto \textsf{pte} \ast  \textsf{pte} \mapsto_{\textsf{vpte,qfrac}} \textsf{pte\_val} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present pte\_val}) \end{array}\right) =\textsf{R}_{\textsf{l1e}} }$
 mov  rax, rdx;
 mov    rdx,-0x28[rbp];
;;if (!pte->present){ 
 mov    -0x28[rbp], rdx
...
;;pte->pfn          = PTE_ADDR_TO_PFN(pa);
 ...
 and    rcx, rdi
 or     rdx, rdi
 mov    rdi, [rsi] (*\label{line:updatepfn}*)
$\specline{ \textsf{pte} \mapsto_{\textsf{vpte,1}} (\textsf{set\_pfn}(\textsf{pte\_val}, \textsf{PTE\_ADDR\_TO\_PFN}(\textsf{fpaddr}))) \ast  \ulcorner \lnot(\textsf{entry\_present pte\_val}) \urcorner \ast P}$
...
... ;;and return either unaccessible pte pointsto or one with fresh page (fpaddr)
 $\specline{ \begin{array}{l}\textsf{pte} \mapsto_{\textsf{vpte,qfrac}} \textsf{pte\_val} \ast \ulcorner (\textsf{entry\_present pte\_val}) \urcorner \ast P \\
 \lor \; \;  \textsf{pte} \mapsto_{\textsf{vpte,1}} (\textsf{set\_pfn}(\textsf{pte\_val}, \textsf{PTE\_ADDR\_TO\_PFN}(\textsf{fpaddr}))) \ast  \ulcorner \lnot(\textsf{entry\_present pte\_val}) \urcorner \ast P \end{array} }$
;;}
\end{lstlisting}
\vspace{-1em}
...
L1 entry (l1e) with the justification of higher levels of the page table exist (R$_{\textsf{walk}}$ in Figure \ref{fig:rwalk}), and for allowing access to the memory of the L1 entry via virtual address (R$_{\textsf{l1e}}$ in Figure \ref{fig:rwalk}).
...
We already discussed for the upper level page-tables that the entry-exists checks are properly ensured ($\textsf{R}_{\textsf{walk}}$ in Figure \ref{fig:rwalk} and Line 27 in Figure \ref{walkpgdir}). However, for L1 level, this check is also checked left to the caller of the traversal function \textsf{walkpgdir}. In other words, unlike  to what we see in R$_{\textsf{walk}}$ for the upper levels where all entry-exists checks are dropped, the specification in R$_{\textsf{l1e}}$ ensures that page table entry for L1 needs to be checked on the caller site. By doing so, as we see in Figure \ref{fig:mapping_code}, the page reference \textsf{fpaddr} is linked to back the virtual address \textsf{va} only if it is not already referring to a physical resource (Lines 16-23 in Figure \ref{fig:mapping_code}). 

The crucial and the only step in addition to traversing the page table in Figure \ref{walkpgdir} is actually updating the L1 entry (Line 35 in Figure \ref{fig:mapping_code}),
via the virtual address
(\textsf{pte}) known to translate to the appropriate physical address, in our example the L1
