...
the appropriate L1 page table entry (Line \ref{line:l1entry_store} in Figure \ref{fig:mapping_code});
...
The crucial step is actually updating the L1 entry (Line \ref{line:l1entry_store} in Figure \ref{fig:mapping_code}),
...
table entry address ($\textsf{addr\_L1}(\textsf{va, l1e})$).
% , to hold the freshly
% allocated physical page address (\textsf{fpaddr}) in Line X.
After this write, we have
...
\[\texttt{pt\_entry+KERNBASE} \mapsto_{\texttt{vpte}}  \; \paddr\; (\textsf{fpaddr \& 0x1})  \quad (\textsf{by PTE variant of}~\TirNameStyle{WriteToVirtMemFromReg})\]

At this point in the proof, % $\texttt{pte\_addr} \mapsto_{\texttt{vpte}}  \; \paddr\; (\textsf{fpaddr})$ 
the virtual PTE points-to
contains  full ownership of the L1 table entry's backing physical memory.
Then, we can split the full-ownership of that
physical pointsto
% ($\textsf{fpaddr}\mapsto_{\textsf{p}} \;\textsf{wzero 64}$) 
into 512 pieces to obtain
fractional ownership of the physical memory appropriate for a single virtual address's share of an L1 entry.
