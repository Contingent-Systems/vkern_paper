...
\end{equation*}
 assuring that looking up identity mappings are safe. The subtle justification relies on the fact that the full ownership asserts the non-presence of the entry, accessing to which requires allocation a new physical page and having the presence bit set. By obtaining the identity mapping for \textsf{entry} needed before Line 7 in Figure \ref{fig:calltopteinitialize}, we have to get it from the $Xi$ with the specification shown in \ref{fig:lookingupidpampping}, relying on the 
 
 \begin{figure*}
   \footnotesize
   \[
 \begin{array}{l}
   \specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{entry} \textsf{\_} \ast }_{\rtv} \\
\specline{  rdi  \mapsto_{\textsf{r}} \textsf{entry\_val} \ast  \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry\_val} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv} \\
\sqsubseteq \\
   \specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{entry\},m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{entry} \textsf{\_} \ast }_{\rtv} \\
 \specline{     rdi  \mapsto_{\textsf{r}} \textsf{entry} \ast \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry\_val} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv} \\
\mathsf{mov} \;   [ \mathsf{r8} ],\mathsf{rdi}\\
\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus \{\textsf{entry}\},m) \ast \textsf{rbp-8} \mapsto_{\textsf{v}} \textsf{entry} \ast r8  \mapsto_{\textsf{r}} \textsf{entry} \textsf{\_} \ast }_{\rtv} \\
\specline{  rdi  \mapsto_{\textsf{r}} \textsf{entry\_val} \ast  \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry\_val} \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}
 \end{array}
 \]
 \vspace{-1em}
