...
\begin{figure*}
\footnotesize
\todo[inline]{C wants to add authoritative ownership of the ghost map of $\Xi$ so talking about the tokens makes sense}
\[
\begin{array}{l}
  ...
  \bigast{(\paddr,\mathsf{T}) \in \Xi}{\exists\; (\textsf{qfrac, q, val,}\vaddr) \ldotp \ulcorner \vaddr = \paddr + \textsf{KERNBASE} \urcorner \ast  \underbrace{\fracghostmaptoken{\delta}{\vaddr}{\paddr}{\qfrac} }_\text{Ghost translation} \ast \underbrace{\paddr \mapsto_{\mathsf{p}}\{\textsf{qfrac}\}\; \vale}_\text{Physical location}}\\
   \underbrace{ \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner}_\text{Entry validity}\ast (\ulcorner\textsf{entry\_present}(val)\urcorner \wand \forall_{\textsf{i}\in\textsf{0..511}} \ldotp ((\paddr + \textsf{i * 8}) \mapsto_{\textsf{id,qfacc}} \_)) \\
  \textsf{ where } \\
...
\todo[inline,color=red]{
  Another Figure 8 issue: last line provide membership in the id map / $\Xi$ not for pa plus whatever, but for \emph{the table base in val} plus whatever,
  also only when the entry is present. I know we whiteboarded this way and sketched it in Zulip this way, and I'm pretty sure I saw something when
  working on issue 19, but need to check w/ Ismail.
}
...
\todo[inline]{some of this content is now earlier, rename to ``installing a new entry'' and just talk about the invariant update from installing the newly-allocated
entry. include explanation of the resources returned from initialization call.
Carefully check speck and then explain code of pte initialize, which currently has not prose (though this is a candidate for appendix in an oopsla submission)
}
