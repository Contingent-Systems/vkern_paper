...
\begin{array}{l}
  ...
  \bigast{(\paddr) \in \Xi}{\exists\; (\textsf{qfrac, q, val,}\vaddr) \ldotp \ulcorner \vaddr = \paddr + \textsf{KERNBASE} \urcorner \ast \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner \\
                           \qquad\qquad\qquad\qquad\qquad\qquad          \underbrace{\sumwalkabs\vaddr\qfrac\paddr }_\text{Ghost translation} \ast 
...
Beyond a frame, the precondition before Line 7 requires the current address space invariant, and knowledge that \textsf{entry} is in the identity-map ($\Xi$) -- $\textsf{paddr} \in \Xi$. By knowing this, we can also refer to the facts in the invariant that are related to a physical page-table addresses is in a range dedicated to page-table entries (as this physical-to-virtual conversion is only guaranteed for page table memory), and check that the addition of the offset (i.e. \textsf{KERNBASE}) to a physical page-table address do not overflow (in \lstinline|rax|).
...
\end{equation*}
assuring that looking up identity mappings are safe. The subtle justification relies on the fact that the full ownership asserts the non-presence of the entry, accessing to which requires allocation a new physical page and having the presence bit set. Acquiring the identity mapping for \textsf{entry} needed before Line 7 in Figure \ref{fig:calltopteinitialize}, we need the knowledge $\textsf{entry} \in \Xi$ to peel-out the fractional token representing the ghost address translation from the offsetted virtual address (\textsf{entry+KERNBASE}) to the physical page address (\textsf{entry}) and physical pointsto relation from the physical address \textsf{entry} to an existentially quantified value (\textsf{entry\_val}) and the side condition (*) we explained above. In in Figure \ref{fig:lookingupidpampping}, we see that all the logical resources needed to construct a virtual-pte pointsto
\[ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry entry\_val} \]
to execute Line 7 in \ref{fig:calltopteinitialize} that can either be mutated or not based on the side condition (*).
 \begin{figure*}
   \footnotesize
   \[
 \begin{array}{l}
...
After obtaining the identity mapping for \textsf{entry}, we are able to load the \textsf{entry\_val} into \textsf{rdi}, and check the presence bit via $and \;0x1,rdi$.
 ...
 In case of absence, \textsf{pte\_initialize} is called for a physical-zeroed page (utilizing the page-allocator's \textsf{kalloc} -- currently the only axiom as it is not part of \textsf{pte} (virtual-memory-management) module) to be backing the physical frame address of the entry -- Specification Line 35 in Figure \ref{fig:calltopteinitialize}. Since we are using \textsf{pte\_initialize} for page-table address allocation, it also ensures us that newly allocated physicall address is in the identity mapping map $\Xi$ -- Line X in Figure \ref{fig:calltopteinitialize}.
\begin{figure}\footnotesize
  \begin{lstlisting}[mathescape]

$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m)   \ast \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{next} \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry\_val}  }_{\rtv}$
$\specline{\texttt{rax} \mapsto_{\textsf{v}} \textsf{\_} \ast \sumapaces\rtv\delta }_{\rtv}$
...
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m)   \ast \texttt{rbp-16} \mapsto_{\textsf{v}} \textsf{next} \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry\_val}  }_{\rtv}$
$\specline{\texttt{rax} \mapsto_{\textsf{v}} \textsf{nextpaddr} \ast \ulcorner \Xi !! \mathsf{nextpaddr} = None \urcorner \ast \sumapaces\rtv\delta }_{\rtv}$
$\sqsubseteq$
...
$\specline{\texttt{rax} \mapsto_{\textsf{v}} \textsf{nextpaddr} \ast \ulcorner \Xi !! \mathsf{nextpaddr} = Z \urcorner \sumapaces\rtv\delta }_{\rtv}$  
mov    rax,-0x10[rbp] 
;;*table = local;
mov    -0x10[rbp],rax
...
\end{figure}
 There two important aspects on the specification after the call to \textsf{pte\_initialize} -- Specification in Figure \ref{fig:pteinitializespec}. First, one might have expected to see a physical pointsto for the fresly page-table address  ($\mathsf{nextpaddr} \mapsto_{\mathsf{p}} \mathsf{w64\_0}$) after the call to \textsf{pte\_initialize} but we do not have this physical pointsto resource. Second, we see the identity mapping for \textsf{nextpaddr} -- $\ulcorner \Xi !! \textsf{nextpaddr} = \textsf{ Some Z} \urcorner$ -- mapped to a different value (\textsf{Z}) than the identity mapping for the physical address \textsf{entry} is (\textsf{V}). The first aspect is important for not invalidating the soundness: \textsf{pte\_initialize} extends the identity map with the freshly allocated \textsf{nextpaddr}, and must not reveal the ownership of the physical pointsto in the post-condition (Line X in Figure \ref{}) because it would create a duplication of the full-ownership of the physical resource in the post-condition would potentially coexists with the one that can be obtained from the side-condition (*) inside the extended invariant in Figure \ref{fig:peraspaceinvariant_with_p2v_extension} with the capability token $\ulcorner \Xi !! \textsf{nextpaddr} = \textsf{ Some Z} \urcorner$. Although, the first aspect recovers the soundness while preserving the knowledge of existence of the identity mapping for the fresly allocated physical page-table address, we still need the knowledge that the fresly allocated address references to a \textit{zeroed} page that an explicitly revealed physical pointsto would give -- $\mathsf{nextpaddr} \mapsto_{\mathsf{p}} \mathsf{w64\_0}$. Since a value of an identity mapping inside the invariant is existentially quantified (\textsf{val} in Figure \ref{fig:peraspaceinvariant_with_p2v_extension}), we would loose the knowledge of identity mapping with zeroed value. To carry this knowledge, we distinguish a random value of an identity mapping -- capability representing the existence of the random value (\textsf{V}) inside the invariant for the address \textsf{entry} at Line X in Figure \ref{} -- from the capability representing the zeroed identity-mapping value zeroed (\textsf{Z}). As we do for the address \textsf{nextpaddr}, when we use the capability with value \textsf{Z} for the identity-map lookup ($\Xi !! \textsf{nextpaddr} = \textsf{ Some Z}$), we know that the value existentially quantified inside the invariant is zeroed -- \textsf{val == w64\_0}.
