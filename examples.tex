...
\begin{figure}\footnotesize
\begin{lstlisting}[mathescape]
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \ulcorner \textsf{KERNELBASE} + \paddr > 0 \urcorner \ast \texttt{rbx}\mapsto_{\textsf{r}} \paddr \ast \sumapaces\rtv\delta  \ast \sumpv\paddr\qfrac\true  }_{\rtv}$
add rbx, KERNELBASE ;; shift physical address by identity mapping constant
$\specline{\exists \qfraczero; \qfrac,\textsf{ w, val}, \textsf{P} \ast \texttt{rbx}\mapsto_{\textsf{r}} \paddr + \textsf{KERNELBASE} }_{\rtv}$
...
\end{lstlisting}
\todo[inline]{Ismail: Should \delta be something else? $\delta$ was used for $\theta$, we need a different gname for $\Xi$.}
\caption{Converting a physical address of a PTE to a virtual address (without instruction pointer or flag updates).}
\label{fig:p2v}
\end{figure}
...
though the Hoare triple we verify for this add instruction is rather complex, with the specification shown in Figure \ref{fig:p2v}.
...
to page table entries (as this physical-to-virtual conversion is only guaranteed for page table memory). The latter uses an additional ghost set added to the per-address-space
...
that physical memory, and also guarantees the offset is the correct calculation.\todo{Colin needs a second pass here}
