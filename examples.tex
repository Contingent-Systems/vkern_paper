...
\end{figure}
Looking at the update from Specfication Lines 5-10 to 10-14 in Figure \ref{walkpgdir}, we see that the removal of \textsf{entry} from the map $\Xi$ in the invariant to construct the \textsf{pte-ptsto} (Specfication Line 13 in Figure \ref{walkpgdir}) needed for executing the load instruction (Line 15 in Figure \ref{fig:calltopteinitialize}) to obtain the value of a page entry whose physical address (\textsf{entry}) kept in the register $\texttt{r8}$. This magical abstraction -- represented as $\Xi$ in the invariant definition -- provides us to construct this \textsf{pte-ptsto} out of an physical address is called \textit{identity-mappings}.
