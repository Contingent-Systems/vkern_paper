...
$\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\;\; \sqsubseteq $
$\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}(\<[\vaddr:=\texttt{fpaddr}]>\theta),\gammaPred) \ast  \texttt{r14}\mapsto_{r} \texttt{pte\_addr} \ast \texttt{rdi}\mapsto_{r} \vaddr \ast \texttt{rax}\mapsto_{r} \_ \; \ast}_{\rtv}$
...
in Specification Line 7 ($\texttt{pte\_addr} \mapsto_{\texttt{vpte}}  \; \paddr\;(\texttt{wzero 64})$), we save it to \textsf{r14} to be updated later in Line 9.
...
Therefore, when we unfold the $\texttt{pte\_addr} \mapsto_{\texttt{vpte}}  \; \paddr\;(\texttt{wzero 64})$ we will be able obtain a full ownerhsip on the L1 table physical pointsto $\paddr \mapsto_{\mathsf{p}} \textsf{wzero 64}$ which can be split to obtain a fractional ownership (q4) on it ($\paddr \mapsto_{\mathsf{p}} \{q4\} \;\textsf{wzero 64}$) to connect it with the L4-L2 physical table pointstos (Specification Lines 4-6) to obtain the complete physical table-walk assertion (L4\_L1\_PointsTo(rtv maddr l4e l3e l2e pa)). 
