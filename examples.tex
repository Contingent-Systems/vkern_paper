...
Concretely speaking, before Line 13 in Figure \ref{fig:calltopteinitialize}, to read the value referenced by physical address \textsf{entry}, first, we open our the addresss-space invariant ($\mathcal{I}\textsf{ASpace}_{\textsf{id}}(\ptablestore,\Xi,m)$), and then obtain the virtual mapping out of the identity mapping $\Xi$ 
\[ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry entry\_val} \]
via using the capability $\Xi \;!!\; \textsf{entry} = Some \textsf{ V}$ under the side condition (*)
\begin{equation*}
 \ulcorner \textsf{qfrac} = 1 \leftrightarrow \; \lnot\textsf{entry\_present }(\vale) \urcorner \tag{*}
\end{equation*}
assuring that looking up an identity mapping is safe under the subtle justification which equates the fact of full ownership to the non-presence of the entry, accessing to which requires allocation a new physical page and having the presence bit set: \textsf{qfrac = 1} is not known for the address \textsf{entry} before the line before Line 13 in Figure \ref{fig:calltopteinitialize} (which makes the entry value accessible not mutable), and can only be known when the entry presence is inspected at Line 19.
 \begin{figure*}
   \footnotesize
   \[
 \begin{array}{l}
...
After obtaining the identity mapping for \textsf{entry}, we are able to load the \textsf{entry\_val} into \textsf{rdi}, and check the presence bit via $and \;0x1,rdi$.
 Accessing the presence bit and checking the value (Line 19 in Figure \ref{fig:calltopteinitialize}) allow  us to exploit the fact in the side condition (*) in such a way that the condition ($\textsf{entry}\mapsto\textsf{present}$ determines whether we take the non-presence path (Lines 19-44 in Figure \ref{fig:calltopteinitialize}) with full ownership ($\textsf{qfrac} = 1$) to set the entry with proper values for the bitfields, e.g., frame showing freshly allocated page and presence bit.
