...
with DMA devices.}
We have verified\todo{todo!} the page table walk corresponding to \lstinline|ensure_L1| under this constant-offset assumption (including allocating missing page table levels).
A detailed proof outline and discussion of this code is given in Appendix \ref{apdx:tablewalk}.\todo{todo}

The critical challenges of verifying this code relate to both proving memory safety of the traversal --- which requires reasoning accurately about 
computing a virtual address for accessing a page table from its physical address --- and peeling out enough resources from the per-address-space
invariant to construct the pte-points-to used in Figure \ref{fig:mapping_code}. Prior work cannot do this.
...
for page tables at each level were given as parameters rather than verifying the conversion from physical addresses to virtual addresses (or even axiomatizing their lookup).
...
temporarily destroying the mappings and restoring them can actually crash the OS. \textsc{CertiKOS} \todo{hard to word since they intentionally don't publicize any
details}
