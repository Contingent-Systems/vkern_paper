...
Note that none of the verification for this function assumes specific page table levels --- logical parameter \textsf{v} represents the level
...
known-valid page table walks ($\textsf{pte\_walkpgdir}$ Line 11 in Figure \ref{fig:mapping_code})  and update the appropriate L1 page table entry (permissions and frame address Lines 17-22 in Figure \ref{fig:mapping_code}).
%\lstset{
...
mapped to a page (assuming $\theta \; !!\; \vaddr = \texttt{None}$). Mapping a fresh
...
\textsf{pte\_walkpgdir}'s postcondition contains virtual \emph{PTE} pointsto assertions ($\mapsto_{\textsf{vpte}}$) %\todo{colin, these are no more plain physical pointstos}
...
By traversing the page-table tree, we obtain virtual page-table entry pointsto relations (Line 21 in Figure  \ref{fig:mapping_code}) for each level down to the L1. 
...
The crucial step is actually updating the L1 entry (Line 22 in Figure \ref{fig:mapping_code}),
via the virtual address known to translate to the appropriate physical address to hold the freshly
allocated physical page address (\textsf{pa}): from Specification Line 27 to 29 by (\textsf{by the PTE variant of Rule}~\TirNameStyle{WriteToVirtMemFromReg}).

In fact, by the application of Rule ~\TirNameStyle{WriteToVirtMemFromReg}, we obtain full ownerhsip of the L1 table entry physical pointsto. Then, we can split the full-ownership of that
physical pointsto to obtain
fractional ownership on it appropriate for a single address's share of an L1 entry (q in Specification Line 29).
...
This is the first proof we know of for mapping a virtual memory page via using virtual memory handles that does not resort to direct reasoning over the operational semantics.
