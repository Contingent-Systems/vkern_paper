...
\label{sec:syntax}
To develop our core logical ideas, we instantiate \iris with a simple language for a stream of instructions
and a logical machine model corresponding to the execution of x86-64 assembly instructions with virtual memory enabled on the
CPU.
%
...
% \end{lstlisting}
% \todo[inline]{I don't think we need this given a more compact version of Figure \ref{fig:pagetables}}
% \vspace{-1em}
...
We model (and later, give program logic rules for) instructions for \del{basic} register \replace{moves}{loads and stores}, and reading and writing memory.
...
Thus, a store to the memory is $\textsf{mov}~[r_m]~r_r$, and a load from memory is $\textsf{mov}~r_r~[r_m]$.
% \[
% \begin{array}{l}
