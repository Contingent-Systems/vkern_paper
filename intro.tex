...
This kind of context-dependent assertion, where a fact may be true in one address space but not others, has a modal flavor%., and manifets itself at many other verification challenges besides the one, virtual-memory-management, we take as a challenge in this paper. For example, Monie et.al. ~\cite{}, introduces a theory for heap management under the existence of garbage collection where  which relies on two pillars: 1. a predicate 
%Despite its modal flavor, up until now, program logic have taken the contingency to be asserted as a qualified-predicate, e.g. \textit{Stackable} ~\cite{}, and built a mechanism, a decorated weakest-precondition, to operated on these certain type predicates. On the other, we, in this paper, propose
%\begin{itemize}
