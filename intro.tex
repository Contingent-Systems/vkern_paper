...
This makes \del{rich} support for verification the virtual memory management subsystem of an OS kernel critical to the correctness of every other piece of an OS or any software running atop it.
...
implemented as an embedded separation logic within \replace{Iris}{\textsc{Iris}}~\cite{jung2018iris}.
...
software page table walks, and \add{a} complete \add{software} page table walk \replace{which backbones}{ --- the backbone of} other essential functionalities of
...
\replace{with more details (treating parts of the kernel
...
address conversion rather than assuming it has already occurred).}{
  in greater detail (e.g., verifying critical and challenging supporting code that was
