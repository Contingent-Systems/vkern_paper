...
Lines \ref{line:finalpieceS}-\ref{line:finalpieceE} in Figure \ref{fig:calltopteinitializeC} (discussed in Section \ref{sec:p2vC}) deal with the cases where no allocation is necessary \emph{or} the allocation has already
...
For each $\paddr\mapsto \textsf{v} \in\Xi$, the invariant tracks virtual points-to justifying that virtual address $\paddr+\textsf{KERNBASE}$ maps to physical address $\paddr$
...
This seemingly simple piece of code has a highly nontrivial correctness argument, which depends critically on detailed invariants on how access to page table
...
The other address-space modality plays a critical role here; \textsf{Pother} would contain these assumptions in the other
address space.
\looseness=-1
