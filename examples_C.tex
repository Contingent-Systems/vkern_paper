...
pte_t *walkpgdir(pte_t *l4, const void *va) {
  $\specline{\textsf{P} \ast \ulcorner\textsf{l4p}=\textsf{rtv}\urcorner\ast\ulcorner\textsf{rtv+KERNBASE}=\textsf{l4}\urcorner \ast \forall_{i\in\textsf{0..511}}\ghostmaptoken{\textsf{id}}{(\textsf{rtv}+i*8)}{\textsf{4}} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}}_{\rtv}$(*\label{line:walkpgdir_pre}*)
  pte_t *l4_entry = &l4[L4Offset(va)]; // Virtual address of L4 entry(*\label{line:start_l4_calcC}*)
  $\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{l4p}\},m) \ast \forall_{i\in\textsf{0..511}\setminus\{\textsf{L4Offset(va)}\}}\ghostmaptoken{\textsf{id}}{(\textsf{rtv}+i*8)}{\textsf{4}}\ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
  ...
  $\specline{ \textsf{l4p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l4p l4e\_val} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l4e\_val})\urcorner}_{\rtv}$    
  ...
  $\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{l4p}\},m)  \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
  ...
  $\specline{ \textsf{l4p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l4p l4e\_val'} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l4e\_val})\urcorner}_{\rtv}$(*\label{line:first_pte_pointstoC}*)
  ...
  $\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{l4p},\textsf{l3p}\},m)  \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
  ...
  $\specline{ \textsf{l4p+KERNBASE} \mapsto_{\textsf{vpte,qfrac4}}\textsf{l4p}\; \textsf{l4e\_val'} \ast \ulcorner  \textsf{qfrac4} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l4e\_val'})\urcorner}_{\rtv}$(*\label{line:ex_l4_vpteC}*)
  $\specline{ \textsf{l3p+KERNBASE} \mapsto_{\textsf{vpte,qfrac3}}\textsf{l3p}\; \textsf{l3e\_val} \ast \ulcorner  \textsf{qfrac3} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l3e\_val})\urcorner}_{\rtv}$(*\label{line:ex_l3_vpteC}*)
...
$\begin{array}{l}\left( \begin{array}{l}
  \ulcorner
    \textsf{l3}+\textsf{L3Offset(va)} = \textsf{l3\_entry}
    \land \textsf{l2}+\textsf{L2Offset(va)} = \textsf{l2\_entry}
    \land \textsf{l1}+\textsf{L1Offset(va)} = \textsf{l1\_entry} \\
    ...
    \textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus\{\textsf{l4p},\textsf{l3p},\textsf{l2p},\textsf{l1p}\},m)  \ast  
      \ghostmaptoken{\textsf{id}}{(\mathsf{l4p})}{\textsf{4}}\ast \ghostmaptoken{\textsf{id}}{(\mathsf{l3p})}{\textsf{3}} \ast \ghostmaptoken{\textsf{id}}{(\mathsf{l2p})}{\textsf{2}}  \ast \\
      \ghostmaptoken{\delta{}s}{\rtv}{\delta} \ast \textsf{l4p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l4p} \; \textsf{l4e\_val}  \ast \\
      \textsf{l3p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l3p}\;\textsf{l3e\_val}  \ast   \textsf{l2p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l2p} \; \textsf{l2e\_val}  \end{array} \right)  =\textsf{R}_{\textsf{walk} } 
    \\ \left(\begin{array}{l}  \textsf{l1p+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{l1p}\;\textsf{l1e\_val} \ast  \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l1e\_val})\urcorner \end{array}\right) =\textsf{R}_{\textsf{l1e}}\end{array} $
\todo[inline]{Colin: These should be 4 different fractions, not all qfrac, right?
And as a presentation question, should the vaddrs in the vpte pointstos use the physical plus KERNBASE or the l$n$\_entry vaddrs?
}
...
\todo[inline]{text around this note depends on precondition to walkpgdir}
...
void vaspace_mappage(pte_t *l4, void *va,uintptr_t fpaddr ) {
  $\specline{\textsf{P} \ast \ghostmaptoken{\mathsf{\textsf{id}}}{\textsf{l4}}{4} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m) \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta} \ast \ulcorner\theta \; !!\; \vaddr = \texttt{None}\urcorner}_{\rtv}$ (*\label{line:mappage_pre}*)
...
\todo[inline,color=cyan]{This is where Colin is stuck. I don't see how we can derive vptes for any of the 512
L4 entries from just a single identity map token for the base of the table (i.e., entry 0).
Surely we must need ${\bigast_{i\in 0..511}}{\ghostmaptoken{\textsf{id}}{(l4p+i*8)}{4}}$, we should discuss live.
}
From the precondition on Line \ref{line:mappage_pre}, we can derive
the precondition of \lstinline|walkpgdir|:\\
\centerline{$\ghostmaptoken{\mathsf{\textsf{id}}}{\textsf{l4p}}{4} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi,m)
\vdash
\ghostmaptoken{\mathsf{\textsf{id}}}{\textsf{l4p}}{4} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus l4p,m)
\ast \textsf{l4p+KERNBASE}\mapsto_{\textsf{vpte,q}}\textsf{l4p}\;
$}\\
\todo[inline,color=cyan]{end confusion}
...
\todo[inline]{Colin will fix up this paragraph: it's right but also kind of weird, will handle during related work pass}
