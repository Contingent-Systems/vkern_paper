...
pte_t *pte_get_next_table(pte_t *entry) {
  ...
  if (!entry->present(*\label{line:read_entry_contentsC}*)(*\label{line:conditional_childrenC}*)(*\label{line:check_entry_present_jumpC}*)) {// If the present bit is zero, need to allocate next level (* \label{line:check_entry_presentC} *)(* \label{line:mask_presentC} *) 
    ...
    $\specline{ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}}  \textsf{entry} \; \textsf{pte\_initialized(pfn\_set(entryv nextpaddr))} \ast \ulcorner  \textsf{qfrac} = 1 \land \lnot(\textsf{entry\_present entry\_val})  \urcorner }_{\rtv}$
  ...
  $\specline{\textsf{P} \ast \mathcal{I}\texttt{ASpace}_{\textsf{id}}(\theta,\Xi\setminus \{ \mathsf{entry}\mapsto\textsf{v} \}),m) \ast \textsf{entry} \mapsto_{\textsf{id}} \textsf{\_} \ast \ghostmaptoken{\delta{}s}{\rtv}{\delta}  }_{\rtv}$
  $\specline{ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry} \; \textsf{(pte\_initialized (entry\_val.pfn))} \urcorner }_{\rtv}$
...
Line 20 also seems like it lost the paddr of the vpte
}
\todo[inline,color=cyan]{yes update logical entry to entryp, which requires also adding a local-variable assertion that entry the variable
holds entryp+KERNBASE}
\vspace{-1em}
...
}\todo[color=cyan]{maybe say something about it feeling like normal asm verification \emph{because} of the virtual points-to enabling
familiar-style proving except for specific manipulation of page tables.
Maybe mention that it's well-beyond the capabilities of prior work because they just never tried? (said nicer...)
}
...
\todo[inline,color=cyan]{APVMappings is authoritative ownership of the ghost map for the current address space's $\Xi$ (identity mappings).
So the last line shouldn't just existentially quantify $\delta$, but also a $\delta'$ since the authoritative map for $\Xi$ should have its own ghost name
(and does in the Rocq)
}
\vspace{-1em}
...
\end{lstlisting}
\todo[inline]{more confusion of entry as program variable holding vaddr
vs entry as logical variable for the paddr, nextpaddr, etc.
update, clarifying that it returns a value for which the mapping holds.
Also, do we need the conditional implication in this postcondition? it alloc'ed from
a region already in Xi.
And shouldn't we be excluding entry (the paddr) from Xi here?
}
\todo[inline,color=cyan]{no, this proof really doesn't care if entry is in or out of Xi, no need to exclude, but maybe worth explaining}
\vspace{-2em}
