...
We will discuss access to the level 4 table later (Section \ref{wlkpgdirC}). But for subsequent levels, the base address of level $n$ must be
fetched from the appropriate entry in the level $n+1$ table.
This is the role of \lstinline|pte_get_next_table| (Figures \ref{fig:calltopteinitializeC}):
%and \ref{fig:p2vC}):
...
Figure \ref{fig:calltopteinitializeC} presents the initial part of the function, which performs the allocation if necessary.
Lines \ref{line:finalpieceS}-\ref{line:finalpieceE} in Figure \ref{fig:calltopteinitializeC} (discussed in Section \ref{sec:p2vC}) deals with the cases where no allocation is necessary \emph{or} the allocation has already
...
\else
Lines \ref{line:mask_presentC}--\ref{line:check_entry_presentC} check
\fi
...
the case for the next level already existing, which is discussed in Section \ref{sec:p2vC} and Figure \ref{fig:p2vC}.
...
pte_t *pte_get_next_table(pte_t *entry) {
  ...
  $\specline{\ulcorner\textsf{entry\_present(entry\_val)}\urcorner \wand \forall_{i\in\textsf{0..511}} \; \ghostmaptoken{\textsf{id}}{\textsf{table\_root(entry\_val.pfn) + i * 8}}{\textsf{v-1}} }_{\rtv} $ (*\label{line:conditional_childrenC}*)
  /*Reading page table entry justified by virtual pte-pointsto*/
  if (!entry->present(*\label{line:read_entry_contentsC}*)) {
    /* Jump if the present bit is not zero; no need to allocate next level (* \label{line:check_entry_presentC} *)(* \label{line:mask_presentC} *) */
    /*Allocation path starts here (*\label{line:pass_addrof_nextC}*)(*\label{line:alloc_path_startC}*)*/
    $\specline{ \textsf{entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{entry entry\_val} \ast \ulcorner  \textsf{qfrac} = 1 \land \lnot(\textsf{entry\_present entry\_val})  \urcorner }_{\rtv}$ (*\label{line:after_concluding_qfrac1C}*)
    ...
    /*Entry value updates:*/
    entry->pfn = nextpaddr; entry->present = 1; (*\label{line:install_new_entryC}*)
  ...
  /*Physical to virtual address mapping code segment starts here (*\label{line:finalpieceS}*)*/
  ...
  uintptr_t next_virt_addr = (uintptr_t) P2V(next_phys_addr); (*\label{line:p2vC}*);
...
and permits storing a new entry (in ellided code around Line \ref{line:install_new_entryC}).
\looseness=-1
...
void pte_initialize(pte_t *entry) {
  /*Allocate a full zeroed page for 512 8-byte entries(*\label{line:call_to_kallocC}*)*/
  pte_t *local = kalloc();
...
(Line \ref{line:install_new_entryC} in Figure \ref{fig:calltopteinitializeC}) after these freshly allocated resources are incorporated to the 
...
in the last lines of \lstinline|pte_get_next_table| shown in Figure \ref{fig:p2vC} (the continuation of Figure \ref{fig:calltopteinitializeC}).
This is a critical piece of the full page table walk verification.
In our small kernel (Line \ref{line:p2vC} in Figure \ref{fig:p2vC}), as in larger kernels, the C macro \texttt{P2V} common to many kernels
...
%\label{fig:p2v}
%\end{figure}
Figure \ref{fig:p2vC} shows the verification of the end of \lstinline|pte_get_next_table| specialized to the case where 
where no allocation was necessary (i.e., the conditional on Line \ref{line:check_entry_presentC} of Figure \ref{fig:calltopteinitializeC} was taken).
...
which is then refined to the assertion on Line \ref{line:childrenC} of Figure \ref{fig:p2vC}.
...
pte_t *walkpgdir(pte_t *pml4, const void *va) {
  ...
  pte_t *pml4_entry = &pml4[PML4EX(va)]; // Virtual address of L4 entry(*\label{line:start_pml4_calcC}*)
  ...
  /*3.Multiply by 8 (byte index), add to pml4 for entry address (*\label{line:end_pml4_calcC}*) */
  ...
  pte_t *pdp = pte_get_next_table(pml4_entry); (*\label{line:first_getnext_callC}*) 
  ...
  $\specline{ \textsf{pml4\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac}} \textsf{pml4\_entry l4e\_val'} \ast \ulcorner  \textsf{qfrac} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l4e\_val})\urcorner}_{\rtv}$(*\label{line:first_pte_pointstoC}*)
  ...
  $\specline{ \textsf{pml4\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac4}}\textsf{pml4\_entry}\; \textsf{l4e\_val'} \ast \ulcorner  \textsf{qfrac4} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l4e\_val'})\urcorner}_{\rtv}$(*\label{line:ex_l4_vpteC}*)
  $\specline{ \textsf{pdp\_entry+KERNBASE} \mapsto_{\textsf{vpte,qfrac3}}\textsf{pdp\_entry}\; \textsf{l3e\_val} \ast \ulcorner  \textsf{qfrac3} = 1 \leftrightarrow \; \lnot(\textsf{entry\_present l3e\_val})\urcorner}_{\rtv}$(*\label{line:ex_l3_vpteC}*)
...
but does not provide a virtual points-to assertion --- because the function must calculate (Lines \ref{line:start_pml4_calcC}--\ref{line:end_pml4_calcC})
...
Line \ref{line:end_pml4_calcC} calculates the virtual address of the relevant entry, and the subsequent view shift
...
(Line \ref{line:first_pte_pointstoC}), with the entry validity and next-level indexing
...
void vaspace_mappage(pte_t *pml4, void *va,uintptr_t fpaddr ) {
  ...
  pte_t *pteaddr = walkpgdir(pml4, a); (*\label{line:call_walkpgdirC}*)
...
The code uses a helper function \lstinline{walkpgdir} (discussed again in Section \ref{sec:traversingC}).
...
A PTE points-to, as we already brifly mentioned, is defined just like the normal virtual points-to of Figure \ref{fig:virtualpointstosharingC}, except the physical address (here, \textsf{pa}) is explicit in the assertion 
...
By doing so, as we see in Figure \ref{fig:mapping_codeC}, the page reference \textsf{fpaddr} is linked to back the virtual address \textsf{va} 
only if it is not already referring to a physical resource (Lines \ref{line:mappage_pte_present_startC}--\ref{line:mappage_pte_present_endC} in Figure \ref{fig:mapping_codeC}). 
...
A call to this code begins executing one thread (until just before Line \ref{line:end_saveC}) in one address space ($\rtv$), whose information will be saved in a structure at address $old$,
and finishes execution executing a different thread in a different address space (Line \ref{line:end_restoreC} on) whose information is initially in $new$.
