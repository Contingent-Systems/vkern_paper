...
\hrule
\todo[inline]{this next (older) text is a little confusing; it talks about the conversion to virtual points-to,
but that happens at the caller, which is the only one that knows which index is needed.
Clean up: the end of this function returns (1) computationally, the virtual address of the base of the level $n-1$ table;
and (2) logically, the resources for the \emph{caller} to convert any aligned virtual address in the 4K
range after that base into a virtual points-to.
}
...
\todo[inline]{Colin says: should make this section more concise, really just tie up earlier discussions.
But make a point to highlight that none of the specifications need to track which address space we're dealing with!}
\hrule
