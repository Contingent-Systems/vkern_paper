...
\todo[inline]{Colin: I think \lstinline|l2_entry| and such are also supposed to be pointers --- they hold the address of an entry.
And they are passed as arguments to pte get next table, which accepts pointers.
}
...
\todo[inline,color=cyan]{WIP checkpoint:
continue by highlighting parallels between assertion on line 13 and precondition of get next table,
and between assertions on lines 4 and 16 and the postcondition.
Then finish talking through essentially the safety argument, then loop back and talk about accumulating the info for
constructing a virtual points-to for va
}
