...
This is the role of \lstinline|pte_get_next_table| (Figure \ref{fig:calltopteinitializeC}):
%and \ref{fig:p2vC}):
...
\ifPLDI
Line \ref{line:conditional_childrenC} checks % In the condensed figure, it's all on one line
\else
Lines \ref{line:mask_present}--\ref{line:check_entry_present} check
\fi
if the entry's ``present'' bit is set.
If it is zero, a new page must be allocated for the next level of the table. Otherwise the code jumps ahead to
the case for the next level already existing (Lines \ref{line:finalpieceS}-\ref{line:finalpieceE} in Figure \ref{fig:calltopteinitializeC}), which is discussed in Section \ref{sec:p2vC}.
...
pte_t *pte_get_next_table(pte_t *entry) {
  ...
  if (!entry->present(*\label{line:read_entry_contentsC}*)(*\label{line:conditional_childrenC}*)\label{line:check_entry_present_jumpC}) {
    ...
    pte_initialize(entry);(* \label{line:call_to_pte_initializeC}(*\label{line:page_of_capsC}*) *)
