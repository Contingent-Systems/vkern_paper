...
pte_t *pte_get_next_table(pte_t *entry) {
  pte_t *next;
  /*setting up the stack for a pte_t *next elided*/
  ...
  $\specline{\ldots \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry} \ast \ldots }$
  $\specline{\ldots \ast \texttt{r8}  \mapsto_{\textsf{r}} \textsf{entry} \ast \ldots }$
  (*\label{line:read_entry_contents}*)$\specline{\ldots \ast \texttt{rdi}  \mapsto_{\textsf{r}} \textsf{entry\_val}  \ast\ldots}$
  /*reading page table entry Justified by virtual pte-pointsto*/
  if (!entry->present) {/* Jump if the present bit is not zero; no need to allocate next level (* \label{line:check_entry_present} *)(* \label{line:mask_present} *) */
...
pte_t *walkpgdir(pte_t *pml4, const void *va) {
  ...
  $\specline{\texttt{rax}  \mapsto_{\textsf{r}} \textsf{table\_root(l4e\_val'.pfn)} }_{\rtv}$
