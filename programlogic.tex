...
\todo[inline]{Colin notes: the \lstinline|addr_L1| mentioned in the paragraph above doesn't appear in Figure 2. paddr isn't mentioned
in the definition of \lstinline|L4_L1_PointsTo| in the figure. Shouldn't there be a pure \lstinline|addr_L1(va,l1e)=paddr| in there?
}
...
\end{figure*}  
  ...
  \and
  \inferrule{ }{\mathsf{GhostMap}(\gamma,\theta) \ast \ghostmaptoken{\gamma}{\mathsf{pa}}{\mathsf{va}} \wand \ulcorner \theta(\mathsf{pa})=\mathsf{Some}(\matsf{va}) \urcorner }
  \end{mathpar}
...
\footnotesize
\[\begin{array}{c}
    [r](\mathsf{P}) \;: \mathsf{vProp} \; \Sigma \; \stackrel{\triangle}{=} \; \lambda \_,\; \mathsf{P}~\mathsf{r} \vspace{0.5em}\qquad
    \textsf{Fact P} \stackrel{\triangle}{=} \;\forall \; ,r \; r' \ldotp  \textsf{P r} \dashv\vdash \textsf{P r'}\\
    \inferrule{}{(\textsf{P} \vdash \textsf{Q}) \vdash  ([r](\textsf{P}) \vdash  [r](\textsf{Q}))}\qquad
    \inferrule{}{[r](\textsf{P} \ast \textsf{Q}) \dashv\vdash ([r](\textsf{P}) \ast [r](\textsf{Q})}\vspace{0.5em}\qquad
    \inferrule{}{[r](\textsf{P}\land \textsf{Q}) \dashv\vdash [r](\textsf{P})\land [r](\textsf{Q})}\\
    \inferrule{}{[r](\textsf{P}\lor \textsf{Q}) \dashv\vdash [r](\textsf{P})\lor [r](\textsf{Q})}\qquad
