...
essentially working with separation logic assertions indexed by a choice of page table root as a $\mathcal{W}_{64}$, which we call $\textsf{vProp }\Sigma$:\footnote{
  ...
  This similarity is real, but the existing lifting does not work with indexed \coq types
...
Our logic is the first to address the existence of multiple address spaces, so also requires
a way to look up
...
approaches~\cite{de2023type} is ongoing work.
