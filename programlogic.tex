...
\[
  \begin{array}{l}
    \hbox{(\TirNameStyle{ModalAddressSpace})} \\ \qquad
         [r](P)  \;\stackrel{\triangle}{=}  (\mathcal{W}_{64} \rightarrow_{\textsf{b}} \mathsf{uPredI} (\mathsf{iResUR} \;\Sigma)) \rightarrow_{\textsf{b}} \mathsf{uPredI } \_ := \lambda \mathsf{P},\; \mathsf{P r}. 
  \end{array}
\]
...
Pragmatically, 
  \begin{enumerate}
  \item a modal context (e.g. a address-space modality $[r]P$), logically, draws the attention to the certain facts in the interest of reasoning (e.g. virtual pointsto relations in the current address-space in Figure \ref{fig:virtualpointstosharing})
