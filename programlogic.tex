...
\paragraph{Physical memory  points-to} Our memory design is shaped around page-table-walk accesses in Figure \ref{} where we need to mask the same address with different masks: 52 bit-masks for page-table-address and 12 bit masks offset masks for locating the entry. With being flexible on decorating these two masks with different the mask-computations, which is needed for different levels of page-tables (e.g. l4M52 and l3M52 for page table addresses of level 4 and 3 respectively), we give the physical-memory the following form
\[ \textsf{w1} \ \sim \ \textsf{w2} \mapsto_{\textsf{a}} \; \{\textsf{q}\} \; \textsf{v} \]
In this form, we see that \textsf{w1} could be an 52-bits masked address to level 4 table ($\textsf{w1 } =( \textsf{ l4M52 maddr cr3val}) $), and, expectedly \textsf{w2} is an address computed by page-offset computation (e.g. $\textsf{l4off maddr cr3val}$). Giving a raw 64-bits memory pointsto assertion becomes
\[\textsf{w} \mapsto_{\textsf{p}} \{q\} \textsf{ v} \stackrel{\triangle}{=} (\textsf{drop 12} w \ \sim \ \textsf{bottom 12 } \mapsto_{\textsf{a}} \textsf{ v} \]
trivially defining \textsf{w1} in terms of \textsf{w} by dropping its 12-bits, and taking the bottom 12-bits of \textsf{w} as \textsf{w2}.
\begin{figure*}
  \begin{lstlisting}[language=Coq]
 Definition $\vaddr\mapsto_{\textsf{t}}\{\textsf{q}\}\; \vpage$ : vProp $\Sigma$ := 
  $\exists_{\textsf{l4e l3e l2e l1}} \ldotp$  $\ulcorner$ aligned $\vaddr \urcorner \ast$ L4_L1_PointsTo(maddr l4e l3e l2e l1e) $\ast$ addr_L1$(\vaddr,\textsf{l1e}) \mapsto_{p} \vpage$.
...
\todo[inline,color=red]{Colin says: shouldn't ther be a fraction q in the body of the virtual pointsto def?
I know this is just a sketch that we then move away from, but it's worth getting accurate.}
\todo[inline,color=cyan]{Colin says: the virtual points-to here is not defined as a vprop! the body is an iProp,
containing an unbound variable rtv, which should be the cr3val taken as an argument. (Also, rtv doesn't make sense to me
as a shorthand, why not cr3val?). I'm also pretty sure the maddr in the virtual points-to definition should be va.}
  \label{fig:strongvirtualpointsto}
\end{figure*}

\todo[inline,color=red]{Colin says: Ismail, I think you accidentally deleted discussion of physical points-tos here?}


\todo[inline,color=red]{Colin says: Why are we talking about assertions for address translations here? It seems like we should
\todo[inline]{Colin says: There's no explanation of the split 52+12 \emph{notation} here. If we added such an explanation
(maybe to where physical points-tos are introduced?) and moved this discussion to \ref{sec:overly-restrictive}, this is otherwise
a nice explanation}
discuss physical memory pointsto assertions in general first, then talk about this in \ref{sec:overly-restrictive}}
...
\begin{itemize}
  \item Level-4 Translation (L4): Performs the first level address translation to get the L4 entry (L4E) in Figure \ref{fig:pagetables} by using the masks l4M52 and l4off with \textsf{rtv} virtual base address to get the starting address of the L4 table (L4T), and locate the entry amongst 512 (q1) ones respectively.
    \begin{lstlisting}[language=Coq]
...
\begin{figure*}
\todo[inline]{Colin says: again, this is supposed to be a vprop, but isn't doing anything with a cr3val argument.
I realize the GASI and per-address-space invariants, and therefore machinery to tie the virtual points-to
to the GASI, hasn't been introduced yet. Is there a short-hand here we can use to be ``more complete'' until the later text
that fills in these details?
}
\todo[inline,color=cyan]{Colin says: Oh!!! I see. You have that extremely subtle rtv subscript on the virtual points-to
syntax! Don't do that. First, it's really subtle and wasn't explained anywhere, so even I missed it. Second,
we're very explicitly describing vProps as functions from root to iProp. Sticking the rtv on the arrow there
is partly weird because it obscures which argument is really the vProp argument (there are 2 other word64s in that syntax),
partly weird because it's written next to something that \emph{isn't} a variable (the v subscript on the virtual points-to
arrow) while using the same font, and partly weird because when you actually write these assertions, rtv doesn't show
up in the syntax at all, which is the whole point --- virtual points-to defs shouldn't mention their cr3 or address space at
all!
Here's what I think it should look like (I also existentially quantified $\delta$ and fixed the p subscript on physical
points-to):
}
\[
\begin{array}{l}
  ...
  \sumwalkabs\vaddr\qfrac\paddr \ast \paddr \mapsto_{\mathsf{p}} \vale
\end{array}
\]
\todo[inline]{Colin says: The fact that we can't visually distinguish the variable v that is the value in memory
from the subscript v on the virtual points-to arrow is not ideal: it makes it hard to tell what's concrete syntax
and what's a variable. This is worth fixing, but near the bottom of the priority list}
...
\end{figure*}  
  ...
  \todo[inline]{Colin says: This requires explaining all the math here. Most reviewers of our paper will not be Iris
  experts. Even the notion of an authoritative RA is something you'd need to explain (the general idea of).
  So: write for non-Iris-expert readers. Above I explained the intuition behind the construction, so now we have to show
  that we correctly implemented that intuition in Iris. So explain authoritative RAs, and how the line below
  implements the intuition above.}
...
\todo[inline]{Colin says: I can rewrite this remark into flowing text after you fully explain the math above.}
...
\todo[inline]{Colin says: I will go back and add some foreshadowing in the explanation of virtual points-to,
which already contains the Aspaces lookup a couple pages back.}
