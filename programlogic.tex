...
\begin{itemize}
  \item Page Map Level-4 Translation (L4): Address the first level address translation to get the PML4E entry in Figure \ref{fig:pagetables} by using the masks(\textsf{va}$|^{52}$(\textsf{CR3}) to get the starting address of the PML4 table and using the offset mask \textsf{va}$|^{12}$(\textsf{CR3}). It is worth noting that our offset indices are 12 bits as all physical memory pointsto assertions has to satisfy being \textsf{\textit{aligned}}, e.g. [47-39] plus 3 alignment bits in Figure \ref{fig:pagetables}. 
    \[ \begin{array}{l}
