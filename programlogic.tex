...
To define and use virtual points-to assertions, we require two more basic assertions:
...
\end{figure*}  
  ...
  resource indeed maps $k$ to $v$. These properties are captured by Iris rules using the token to update or draw conclusions about the logical map:\footnote{Iris ghost maps lack established notation in the literature,
...
Our logic is the first to address the existence of multiple address spaces, so requires
a way to look up
the ghost map of virtual-to-physical translations for an address space (the ghost name $\delta$ in Figure \ref{fig:virtualpointstosharing}).
...
\inferrule[\footnotesize WriteToRegFromVirtMem]{
  \left\{ \begin{array}{l}
    ...
    r_d \mapsto_{r}  \textsf{v} \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v,rtv}} \textsf{v}
...
address space back into the \emph{new} address space, where they may not hold. Appendix \ref{sec:issues} gives more details.
...
convenient ways to discard the current continuation. We are working to adapt existing machinery~\cite{de2023type} to prove their soundness.
