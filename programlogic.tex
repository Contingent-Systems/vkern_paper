...
\begin{figure}
\begin{mathpar}
    [r](\mathsf{P}) \;: \mathsf{vProp} \; \Sigma \; \stackrel{\triangle}{=} \; \lambda \_,\; \mathsf{P}~\mathsf{r} \and
    \textsf{Fact P} \stackrel{\triangle}{=} \;\forall \; ,r \; r' \ldotp  \textsf{P r} \dashv\vdash \textsf{P r'}\\
    \inferrule{}{(\textsf{P} \vdash \textsf{Q}) \vdash  ([r](\textsf{P}) \vdash  [r](\textsf{Q}))}\and
    \inferrule{}{[r](\textsf{P} \ast \textsf{Q}) \dashv\vdash ([r](\textsf{P}) \ast [r](\textsf{Q})}\and
    \inferrule{}{[r](\textsf{P}\land \textsf{Q}) \dashv\vdash [r](\textsf{P})\land [r](\textsf{Q})}\and
    \inferrule{}{[r](\textsf{P}\lor \textsf{Q}) \dashv\vdash [r](\textsf{P})\lor [r](\textsf{Q})}\and
...
\todo[inline]{Ismail: Can we chat Wednesday about this rule, which has cr3 register pointstos instead of the cr3 subscripts?
The subscript form might make the derivation of a frame rule harder.}
\begin{mathpar}
...
\todo[inline]{Ismail: Please update this section with rip stuff}
...
% \\
 \inferrule[AddRegImm]{
   \{P \ast (r_d \mapsto_{r}  \textsf{rvd+imm} \lor  r_d \mapsto_{r} 0 ) \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{t}} \textsf{v} \}_{\textsf{rtv}}\;\overline{is}
 }{
   \{P \ast \textsf{rip} \mapsto_{\textsf{r}} \textsf{iv} \ast \textsf{rflags} \mapsto_{\textsf{r}} \textsf{flgs} \ast \ulcorner (\textsf{rvd + imm} > 0 \lor \textsf{rvd + imm} = 0 ) \urcorner \ast r_d \mapsto_{r}  \textsf{rvd} \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{t}} \textsf{v} \}_{\textsf{rtv}}
   \textsf{add}~\textsf{r}_d,~[\textsf{imm}];\;\overline{is}
 }
 \\
\inferrule[WriteToRegFromVirtMem]{
  \{P \ast \mathcal{I}\textsf{ASpace}\ast \mathcal{I}\textsf{ASpace} \ast \textsf{rip} \mapsto_{\textsf{r}} \textsf{iv + x} \ast \ulcorner x = 3 \lor x =4 \urcorner \ast r_d \mapsto_{r}  \textsf{v} \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v,rtv}} \textsf{v} \}_{\textsf{rtv}}\;\overline{is}
}{
...
\inferrule[WriteToVirtMemFromReg]{
  \{P \ast \mathcal{I}\textsf{ASpace}\ast \textsf{rip} \mapsto_{\textsf{r}} \textsf{iv + x} \ast \ulcorner x = 3 \lor x =4 \urcorner \ast r_s \mapsto_{r}\{q\}  \textsf{rvs}  \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v}} \textsf{v} \}_{\textsf{rtv}}\;\overline{ is}  
}{
...
At the moment our proofs do rely on 11\todo{update!} small axioms of properties which should be provable, but
