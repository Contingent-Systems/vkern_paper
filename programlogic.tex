...
Every modal logic can be translated straitforwardly to a logic without modalities
...
This becomes most apparent when discussing our proof rules.
}
...
\footnotesize
  ...
  is what allows stating the global kernel invariants needed for software page traversals, which prior work did not
...
\inferrule[\footnotesize WriteToRegFromVirtMem]{
  \left\{ \begin{array}{l}
    P \ast \mathcal{I}\textsf{ASpace}\ast \mathcal{I}\textsf{ASpace} \ast \textsf{rip} \mapsto_{\textsf{r}} {\textsf{iv} + \textsf{MovLen}(\textsf{r}_d,[\textsf{r}_a])} \ast \arcr
    r_d \mapsto_{r}  \textsf{v} \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v}} \textsf{v}
...
conclusion). 
It reflects the load of that memory value into the destination register $\textsf{r}_d$, with
...
the current address space.
\todo[inline,color=green]{WIP: Colin is trying to clarify the value of modal approach here}
}
