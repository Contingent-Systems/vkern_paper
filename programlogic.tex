...
\todo[inline]{explain $\Theta$, $m$, and figure out where $\delta$ is bound in Figure \ref{fig:peraspaceinvariant}}
...
\caption{Global Address-Space Invariant with a fixed global map of address-space names $m$}
  ...
  % Colin removed this remark because it's skwtching a *3rd* alternative design for virtual points-to,
  % which could be useful for some readers, but will most likely just confuse
...
\todo[inline]{Colin has not looked at this section in detail yet.
It is clear the discussion of the cr3 update needs updating for doubles}
...
\[
  \begin{array}{l}
    ...
    \hbox{(\TirNameStyle{ModalAddressSpaceMono})} \qquad (\mathsf{P} \vdash \mathsf{Q}) \vdash  ([r](\mathsf{P}) \vdash  [r](\mathsf{Q}))\\
    \hbox{(\TirNameStyle{ModalAddressSpaceSep})} \qquad [r](\mathsf{P} \ast \mathsf{Q}) \dashv\vdash ([r](\mathsf{P}) \ast [r](\mathsf{Q})\\
    \hbox{(\TirNameStyle{ModalAddressSpacePure})} \qquad  \mathsf{pure\_vprop P}(\mathsf{iPropI }\Sigma)\textsf{ : vProp }\Sigma \stackrel{\triangle}{=} \; \lambda \_ \ldotp \mathsf{P}\\
    \hbox{(\TirNameStyle{ModalAddressSpaceFact})} \qquad  \mathsf{Fact P}(\mathsf{vProp }\Sigma) \stackrel{\triangle}{=} \;\forall \; ,r \; r' \ldotp  \mathsf{P r} \dashv\vdash \mathsf{P r'}\\
    \hbox{(\TirNameStyle{ModalAddressSpaceFactIntroElim})} \\
    \qquad\qquad \mathsf{Fact pure\_vprop P} \vdash  ([r](\mathsf{pure\_vprop P}) \dashv\vdash (\mathsf{pure\_vprop P}) (\mathsf{vProp }\Sigma))\\
    
    \hbox{(\TirNameStyle{ModalAddressSpaceAnd})} \qquad   [r](\mathsf{P}\land \mathsf{Q}) \dashv\vdash [r](\mathsf{P})\land [r](\mathsf{Q})\\    
    \hbox{(\TirNameStyle{ModalAddressSpaceOr})} \qquad    [r](\mathsf{P}\lor \mathsf{Q}) \dashv\vdash [r](\mathsf{P})\lor [r](\mathsf{Q})\\   
    \hbox{(\TirNameStyle{ModalAddressSpaceFactIntroElim})} \\
    \qquad\qquad \mathsf{Fact pure\_vprop P} \rightarrow  ([r](\mathsf{pure\_vprop P}) \dashv\vdash (\mathsf{pure\_vprop P}) (\mathsf{vProp }\Sigma))\\
