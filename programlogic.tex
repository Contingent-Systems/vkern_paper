...
\begin{lstlisting}[language=Coq]
  Definition vProp  $\Sigma$ : bi := word 64 $\rightarrow$_{b} iPropI  $\Sigma$.
  \end{lstlisting}
...
\todo[inline]{We should drop all talk about PDE, PDPT, etc., and just use L1-L4}
...
\begin{itemize}
  ...
  \item Page Table Level Translation (L1): Performs the second level address translation to get the PT entry (PTE) in Figure \ref{fig:pagetables} by using the masks(\textsf{va}$|^{52}$(\textsf{PDE}) to get the starting address of the Page table (PT) and using the offset mask \textsf{va}$|^{12}$(\textsf{PDE})
    \[ \begin{array}{l}
      ...
      \exists \entryo \ldotp \nfpointsto{\mask\vaddr\ft\entrytw}{\mask\vaddr\tw\entrytw}\entryo\qfour\naddr
      \end{array}
      \]
...
\begin{figure*}
 ...
 Definition $\vaddr\mapsto_{\textsf{t}}\{\textsf{q}\}\vpage$ :vProp $\Sigma$ := 
  $\exists_{\entryf \;, \entrytr \;, \entrytw \;,\entryo} \ldotp$  $\ulcorner$ aligned $\vaddr \urcorner \ast$ L4_L1_PointsTo($\rtv,\entryf,\entrytr,\entrytw,\entryo$) $\ast$ addr_L1$(\vaddr,\entryo) \mapsto_{p} \vpage$.
 Definition L4_L1_PointsTo (rtv maddr entry entry0 entry1 pte :word 64) : vProp $\Sigma$ :=
  ((l4M52 maddr rtv) \$\sim$\ (l4off maddr rtv) $\mapsto_{a}$ {q1} entry  $\ast$
  (l3M52 maddr entry) \$\sim$\ (l3off maddr entry)  $\mapsto_{a}$ {q2} entry0 $\ast$ 
  (l2M52 maddr entry0) \$\sim$\ (l2off maddr entry0) $\mapsto_{a}$ {q3} entry1 $\ast$
  (l1M52 maddr entry1) \$\sim$\ (l1off maddr entry1) $\mapsto_{a}$ {q4} pte ).
\end{lstlisting}
