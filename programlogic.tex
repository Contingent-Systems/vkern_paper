...
The choice modal context itself already exhibits itself as a reasoning principle (Figure \ref{fig:modalcontextinvariance}) in which we can intro/eliminate any resource that is not virtual pointsto into/from modal context without dischargin any obligation. We can use this entailment for  \todo[inline,color=red]{Ask Colin}.
\begin{figure}
  \[
  \begin{array}{l}
\hbox{(\TirNameStyle{ModalContextInvariance})} \qquad
 [r](Q \star \ell \mapsto_{\textsf{p}} \vpage)  \;\vdash [r](Q) \star \ell \mapsto_{\textsf{p}} \vpage 
  \end{array}
  \]
\caption{Modal Context Invariance}
\Description{Modal Context Invariance}
\label{fig:modalcontextinvariance}
\end{figure}
\begin{remark}[The Choice of Modal Context as a Pattern of Verification]
  By choosing the contingency and the modal contex, we simply introduce 
  \begin{enumerate}
  \item the pattern of what the essential abstraction (e.g. address spaces) we care about to do verification is
  \item how a modal context (e.g. a address-space modality), logically, draws the attention to the certain facts in the interest of reasoning (e.g. virtual pointsto relations in the current address-space)
  \item and, offloads the burden of individual bookkeeping of these facts being interesting under different context (e.g. virtual pointsto relations in other address-spaces) via utilizing the \textit{summarization} aspect of the contingency it represents (e.g. other virtual pointsto relations are valid under other certain address-space page-table root addresseses).  
  \end{enumerate}
\end{remark}
\paragraph{Interaction}
\label{sec:interaction}
After defining the modal context, we can now use introduction/elimination of a resource into/from a modal context for the sake of real low-level systems verification. For example, we can extend the modal context with some new virtual address mapping after backing them with new pages...

After defining the mod 
This rule captures the notion that switching address spaces pulls information out of one modality into the \textit{“current view”} of memory, and leaves other assertions true relative to the previous address
space.
