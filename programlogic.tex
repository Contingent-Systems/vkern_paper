...
\[
\{ \crthree \mapsto_{r} r0 \star \textsf{va1} \mapsto_{v} \textsf{v}_1 \star \textsf{va2} \mapsto_{v} \textsf{v}_2 \star \textsf{va3} \mapsto_{v} \textsf{v}_3 \star [r1]\;P \}
\]
where we see the \textit{current address-space} rooted at $r0$ is loaded into the  \textit{current-view} of the memory with its resources ($ \textsf{va1} \mapsto_{v} \textsf{v}_1 \star \textsf{va2} \mapsto_{v} \textsf{v}_2 \star \textsf{va3} \mapsto_{v} \textsf{v}_3$), and framed by the address-space rooted at $r1$ with resources $P$. 
...
\todo{inlined, color= red}[STOP HERE, go back fix the stuff discussed with COLIN]
\begin{figure}
\begin{mathpar}
\inferrule[WriteToMemoryFromReg]{
