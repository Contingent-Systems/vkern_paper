...
In the rest of this, we explain how specifications of some selected \textsf{AMD64} instructions look like under the decisions made on the reasoning principles.
\begin{figure} 
  \[
  \begin{array}{l}
...
\inferrule[WriteToVirtMemFromReg]{
  \{P \ast r_s \mapsto_{r}\{q\}  \textsf{rvs}  \ast r_a \mapsto_{r} \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v,rtv}} \textsf{v} \}\;\overline{ is}  
}{
  \{P \ast r_s \mapsto_{r}\{q\}  \textsf{rvs}   \ast r_a \mapsto_{r}\{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v,rtv}} \textsf{rvs} \}
  \textsf{ mov}~\textsf{r}_a,~\textsf{r}_s;\;\overline{is}
}
\\
...
\subsubsection{Register To Memory}\todo[inline,color=red]{Iso:explain}
 \paragraph{Register To Physical Memory}\todo[inline,color=red]{Iso:explain}
\paragraph{Register To Virtual Memory} \todo[inline,color=red]{Iso:explain}
\subsubsection{Register To Register}\todo[inline,color=red]{Iso:explain}
\paragraph{Updating \lstinline|cr3|}\todo[inline,color=red]{Iso:explain}
