...
The definition in Figure \ref{fig:strongvirtualpointsto} defines a virtual points-to assertion as
existentially quantifying the physical memory addresses of the individual entries in each page table,
asserting that a corresponding page table walk exists (\lstinline|L4_L1_PointsTo|), and then 
\todo[inline]{snapshot... the last bit of that assertion is weird, but I'll fix. I'll also just remove the fraction from that def since it gets
ignored anyways and we provide a better version shortly}
