...
value of the register $\rv$.\todo{Ismail, you changed ``the register value $\rv$'' to ``value of the register $\rv$'' and I changed it back. What you wrote treats $\rv$ as the name of the register, not its value.}
...
For example, the first-level address translation to get the L4 entry (\lstinline|l4e|) 
  ...
  Then \del{at} that physical location \replace{is}{holds} the appropriate entry in the L4 table \textsf{l4e}.
...
and thus that the per-address-space invariant will contain the physical resources that guarantee \add{that} the hardware resolves the translation.
