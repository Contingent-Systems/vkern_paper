...
\add{Since \textsf{vProp}s are ``just'' \textsc{Iris} predicates on a choice of page table root,
...
This becomes more apparent when discussing our proof rules.
}
...
\centerline{$\textsf{w} \mapsto_{\textsf{p}} \{q\} \textsf{ v} \stackrel{\triangle}{=} (\textsf{drop 12}~w) \ \sim \ (\textsf{bottom 12}~w)\mapsto_{\textsf{a}} \; \{\textsf{q}\} \textsf{ v} $}
...
\caption{A Strong Virtual Points-to Relation
  \todo[inline]{Ismail: Tie these lines to the steps and entries in Figure \ref{fig:pagetables}, e.g., with \textcircled{1}\ldots}
  \todo[inline]{Ismail Addressed: earlier in the background and the current text is clearly related to \ref{fig:pagetables}}
}
...
\todo[inline]{This is a section where it's worth explaining the figure in a little more detail, especially as we reuse L4L1PointsTo
in our own definitions later. I know we discussed at some point, but don't the \texttt{l$n$M52} accessors ignore the \texttt{maddr},
and don't the \texttt{l$n$off} accessors ignore the entry arguments? If so we can drop those (simplifying the figure), and annotate 
(e.g., with comments like \lstinline[mathescape]|(*$\textcircled{E}$*)|) each \texttt{l$n$off} in Figure \ref{fig:strongvirtualpointsto}
with a tag matching a new tag on the corresponding offset chunk of the translated virtual addres in Figure \ref{fig:pagetables}\ldots
}
\todo[inline]{Ismail Addressed: current explanation in here is fortified by the changed text in the background section smoothly}
...
Our solution \replace{removes}{in the next subsection improves on theirs by removing} some restrictions in this definition by further abstracting the handling of address translation.
...
\todo[inline]{What's not clear in this next paragraph is the relationship between ASpaceLookup in Figure 5 and the lookup part of Figure 3.
I think Figure 5's $m$ and Figure 3's $\delta{s}$ are the same, aren't they? If so, we should say so and use the same name.
If not we should explain why they're different.
}
...
\inferrule[\footnotesize WriteToVirtMemFromReg]{
  \{P \ast \mathcal{I}\textsf{ASpace}\ast \textsf{rip} \mapsto_{\textsf{r}} \textsf{ iv} + \textsf{MovLen}(\textsf{r}_d,[\textsf{r}_a])  \ast r_s \mapsto_{r}\;\{q\}  \textsf{ rvs}  \ast r_a \mapsto_{r} \; \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v}} \textsf{ v} \}_{\textsf{rtv}}\;\overline{ is}  
}{
  \{P \ast \mathcal{I}\textsf{ASpace}\ast \textsf{rip} \mapsto_{\textsf{r}} \textsf{ iv} \ast r_s \mapsto_{r} \;\{q\}  \textsf{ rvs}   \ast r_a \mapsto_{r} \; \{q\} \textsf{ vaddr} \ast \textsf{vaddr} \mapsto_{\textsf{v}} \textsf{ rvs} \}_{\textsf{rtv}}
  \textsf{ mov}~[\textsf{r}_a],~\textsf{r}_s;\;\overline{is}
}
\\
...
% }
\end{mathpar}
\todo[inline]{Ismail, what do you think about shading the ``core'' assertions that are common to any asm program logic?
Or shading the VMM-specific ones?
}
\todo[inline]{Ismail Addressed: I don't think that we should do -- general instructions' text outlines the how to read VMM related ones in the text. 
}
\vspace{-1em}
...
conclusion). 
It reflects the loading of that memory value into the destination register $\textsf{r}_d$, with
...
model updates to the instruction pointer \lstinline|rip| \add{based on the length of the encoded instruction.
These rules} \replace{using}{use} a metafunction \textsf{MovLen}.
...
the current address space.
\todo[inline,color=green]{WIP: Colin is trying to clarify the value of the modal approach here}
}
\todo[inline,color=cyan]{Colin says:
Ismail, Figure 7 looks broken. Why are there two IAspaces in WriteToRegFromVirtMem? Shouldn't there
only be one?
Also in WriteToRegFromVirtMem, why can't the virtual points-to be fractional? It is in the Rocq development,
and we use it as such in examples.
In WriteToVirtMemFromReg, vaddr points to a value rvs both before and after the store.
And also in WriteToVirtMemFromReg, why are all the fractions q? They should be allowed to be different fractions.
(Also, explaining the fraction notation was one of the questions from reviewers.)
}
\todo[inline,color=cyan]{Ismail Addressed:
Others issues are addressed.
"(Also, explaining the fraction notation was one of the questions from reviewers.)"
They are explained earlier in the text. No need. They should go back and read.
}
...
\del{bare} assertions \add{for the current address space ($P$ and $\mathcal{I}\mathsf{ASpace}$)} under an other address-space modality for \add{the initial
...
\todo[inline,color=green]{Colin needs to add more of the ``why modals'' stuff here, per the response.
But it's a little awkward: in the response we said this was hard to specify with ghost-tagged
pointers, but now I'm wondering if that's \emph{quite} right.
It seems like maybe you could just plumb an implicitly-tagged P through to the following
instructions and be okay, the instantiation of P would just be horrid.
What we said about e.g., defining
a generic linked list is still true, and should be raised earlier (not here).
Here, we want to make a point about packaging all resources for a specific address space
together. But it would be stronger if we could hone the argument in the response more.
The bit about ``how would we talk abstractly about the fact that some unspecified resources R should
be sufficient ... in the new address space'' is the right bit to polish...
ghost bit discussion should either go in relwork with some Dafny(?) discussion, or earlier in motivation.
}
